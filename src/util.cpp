...
const char * const BITCOIN_CONF_FILENAME = "billionaercoin.conf";
const char * const BITCOIN_PID_FILENAME = "billionaercoin.pid";
...
#else
    const char* pszModule = "billionaercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BillionaerCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BillionaerCoin";
#else
    // Unix
    return pathRet / ".billionaercoin";
#endif
#endif
}
